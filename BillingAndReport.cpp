...
public:
    double calculateTotalBill(const vector<pair<string, int>> &orderList)
    {
        ...
        while (getline(menuFile, line))
        {
            ...
            for (const auto &item : orderList)
            {
                ...
                {
                    totalBill += itemPrice * item.second;
                    break;
                }
            }
        }
...
public:
    void GenerateBill(const vector<pair<string, int>> &orderList)
    {
        ...
        cout << "----------------------------------------------" << endl;
        cout << setw(30) << left << restaurantName << "Date: " << put_time(localTime, "%d %b %Y") << endl;
        cout << "----------------------------------------------" << endl;
        ...
        salesReport << put_time(localTime, "%d %b %Y") << "," << totalBill << endl;
        salesReport.close();
    ...
    void CalculateProfit(const vector<pair<string, int>> &orderList)
    {
    ...
    void GenerateCustomerFeedbackReports()
    {
        try
        {
            ...
            while (getline(feedbackFile, line))
            {
                ...
                stringstream ss(line);
                char comma; // To read the comma separator between year and rating
                if (!(ss >> year >> comma >> rating))
                {
                    throw invalid_argument("Invalid format in feedback data.");
                }
                getline(ss, comment); // Read the rest of the line as comment
